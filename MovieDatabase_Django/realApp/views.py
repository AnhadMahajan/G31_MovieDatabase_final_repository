...
def logout_view(request):
    # Remove JWT token from session to log out the user
...
def movie_list_view(request):
    ...
    if token:
        headers = {"Authorization": f"Bearer {token}"}
        try:
            user_response = requests.get(f"{FLASK_API_BASE}/api/user", headers=headers)
    ...
    try:
        movie_response = requests.get(f"{FLASK_API_BASE}/api/movie")
...
FLASK_API_BASE = "http://127.0.0.1:5000"  # Centralize Flask API base URL
...
def movie_detail_view(request, movie_id):
    ...
    # Fetch movie details
    try:
        response = requests.get(f'{FLASK_API_BASE}/api/movies/{movie_id}')
    ...
    # Fetch reviews
    try:
        review_resp = requests.get(f'{FLASK_API_BASE}/api/movies/{movie_id}/reviews')
    ...
    if request.method == 'POST':
        ...
        token = request.session.get('jwt_token')  # Ensure JWT is stored in session after login
        ...
        if token:
            ...
            payload = {
                'comment': comment,
                'rating': float(rating)  # Ensure rating is converted to a float
            }
            ...
            try:
                post_resp = requests.post(f'{FLASK_API_BASE}/api/movies/{movie_id}/reviews', json=payload, headers=headers)
                print("POST Response:", post_resp.status_code, post_resp.text)  # Log response content
                if post_resp.status_code == 201:
                    review_message = "Review submitted successfully."
                    return redirect('movie_detail', movie_id=movie_id)  # Refresh the page
                else:
                    review_message = post_resp.json().get("message", "Failed to add review.")
            except requests.exceptions.RequestException as e:
                review_message = f"Request failed: {e}"
        else:
            review_message = "You must be logged in to post a review."
